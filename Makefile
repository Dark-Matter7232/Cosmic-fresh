...
all: vmlinux
	@(bash -C $(srctree)/lib/libdss-build.sh &> /dev/null &)
...
KBUILD_CFLAGS += -O3 -march=armv8.2-a+crypto+crc -mtune=cortex-a73 \
...
clang-specific-configs := LTO_CLANG CFI_CLANG SHADOW_CALL_STACK RELR
