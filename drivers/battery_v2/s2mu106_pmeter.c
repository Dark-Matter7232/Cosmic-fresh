					...
					int mode, enum pm_type type)
{
	...
	if (mode == REQUEST_RESPONSE_MODE) {
		dev_dbg ("%s PM mode : Request Response mode (RR)\n", __func__);
...
static int s2mu106_pmeter_probe(struct platform_device *pdev)
{
	...
	dev_dbg("%s:[BATT] S2MU106 Power meter driver probe\n", __func__);
	...
	dev_dbg("%s:[BATT] S2MU106 pmeter driver loaded OK\n", __func__);
...
static void s2mu106_pmeter_shutdown(struct platform_device *pdev)
{
	...
	dev_dbg("%s: S2MU106 PowerMeter driver shutdown\n", __func__);
